...
class LLMReranker:
    ...
    def set_up_llm(self):
        ...
        if base_url:
            llm = OpenAI(api_key=os.getenv("OPENAI_API_KEY"), base_url=base_url)
        else:
    ...
    def get_rank_for_single_block(self, query, retrieved_document):
        ...
        if self.usage_stats:
            self.usage_stats.record_llm_rerank(1)

        response = completion.choices[0].message.parsed
        if response is None:
            return {"relevance_score": 0.0, "reasoning": "LLM structured parse returned None"}
    ...
    def get_rank_for_multiple_blocks(self, query, retrieved_documents):
        ...
        if response is None:
            return {
                "block_rankings": [
                    {"relevance_score": 0.0, "reasoning": "LLM structured parse returned None"}
    ...
    def rerank_documents(self, query: str, documents: list, documents_batch_size: int = 4, llm_weight: float = 0.7):
        ...
        if documents_batch_size == 1:
            def process_single_doc(doc):
                ...
                vec_sim = max(0.0, min(1.0, 1.0 - float(doc.get("distance", 0.0))))
                doc_with_score["combined_score"] = round(
                    llm_weight * ranking["relevance_score"] + vector_weight * vec_sim,
                    4
                )
            ...
            def process_batch(batch):
                ...
                for doc, rank in zip(batch, block_rankings):
                    ...
                    vec_sim = max(0.0, min(1.0, 1.0 - float(doc.get("distance", 0.0))))
                    doc_with_score["combined_score"] = round(
                        llm_weight * rank["relevance_score"] + vector_weight * vec_sim,
                        4
                    )
