...
_DEFAULT_OUT = BENCHMARK_ROUND1_MINI_DIR.relative_to(ROOT).as_posix()
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Skip this many questions from the start of challenge questions.json (use with --num to slide the window).",
    )
    parser.add_argument("--out-dir", type=str, default=_DEFAULT_OUT, help="Output benchmark directory")
    ...
    n = max(1, min(args.num, len(questions)))
    off = max(0, args.offset)
    if off >= len(questions):
        raise SystemExit(f"--offset {off} is >= total questions ({len(questions)})")
    end = min(off + n, len(questions))
    selected_q = questions[off:end]
    if len(selected_q) < n:
        print(
            f"[WARN] Only {len(selected_q)} question(s) available (offset={off}, requested num={n}); "
            f"challenge has {len(questions)} total.",
            file=sys.stderr,
        )
